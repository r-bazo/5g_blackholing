...
var numPacketsMap = make(map[string]int)
...
func Dispatch(conn net.Conn, msg []byte) {
	...
	*/
	...
	// first do a simple telemetry of counting requests per client
	if val, ok := numPacketsMap[conn.RemoteAddr().String()]; ok {
	        numPacketsMap[conn.RemoteAddr().String()] = val + 1
	    } else {
	        numPacketsMap[conn.RemoteAddr().String()] = 1
	    }

	for key, value := range numPacketsMap {
	    Ngaplog.Infof("[TELEMETRY] connection %s number of requests %d", key, value)
	}
	...
	*/
	...
	if shouldBlackhole(conn) {
		remoteAddr := conn.RemoteAddr().String()
        	Ngaplog.Infof("[BLACKHOLE] Detected DoS from client %s, blackholing...", remoteAddr)
	        _ = conn.Close()
        	return
	}
...
func shouldBlackhole(conn net.Conn) bool {
        remoteAddr := conn.RemoteAddr().String()
        if _, ok := numPacketsMap[remoteAddr]; ok {
                var values []int
                for _, value := range numPacketsMap {
